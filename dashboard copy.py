...
class GitHubAnalysisDashboard:
    def __init__(self):
        """대시보드 초기화"""
        # 초기화 단계에서 load_data 호출하지 않음
        self.data = {}  # 빈 딕셔너리로 초기화
        self._load_data()  # 새로운 개인 메서드 호출
    
    @staticmethod
    @st.cache_data
    def _load_data(self):
        """데이터 로드 및 self.data에 저장"""
        data = {}
    ...
    # def load_data(self):
    #     """분석 결과 데이터 로드"""
    #     self.data = GitHubAnalysisDashboard.load_data()
    def load_data(self):
        """데이터 다시 로드 (필요시)"""
        self._load_data()
    ...
    def show_developer_patterns(self):
        ...
        # 상위 10명의 개발자 선택
        top_devs = self.data['dev_stats'].sort_values('commit_count', ascending=False).head(10)
        ...
        fig = px.bar(
            ...
            labels={'commit_count': '커밋 수', 'index': '개발자'},
            color='commit_count',
...
# 개발자 상세 분석
        ...
        if selected_dev:
            # 선택된 개발자 데이터 가져오기
            dev_data = self.data['dev_stats'].loc[selected_dev]
    ...
    def show_time_patterns(self):
        ...
        if 'day_hour_counts' in self.data and not self.data['day_hour_counts'].empty:
            ...
            weekend_data = heatmap_data.iloc[5:].sum().sum()  # 토-일
            ...
            work_time_data = heatmap_data.iloc[:, work_hours].sum().sum()
